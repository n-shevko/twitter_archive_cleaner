...
def rel_url(media_path, document_path):
    ...
    # os.path.relpath(media_path, os.path.split(document_path)[0]).replace("\\", "/")
    return media_path.split('parser-output')[-1][1:].replace("\\", "/")
...
def convert_tweet(tweet, username, media_sources, users: dict, paths: PathConfig):
    ...
    body_html = header_html + body_html + f'<footer>{formatted_date}&nbsp<a href="{original_tweet_url}" target="_blank">original</a></footer>'
   ...
   """
    ...
    for tweets_js_filename in paths.files_input_tweets:
        ...
        for tweet in json:
            ...
            if 'in_reply_to_status_id' in tweet2 or 'RT @' in tweet2['full_text']:
                # remove author's responses and retwits
                continue
